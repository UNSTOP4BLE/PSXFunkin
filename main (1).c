...
// Timer resolution = (64.59 * 2^TIMER_SHIFT) Hz
...
void cd_read_handler(int event, uint8_t *payload) {
	...
	str_ctx.state = (event == CdlDataReady) ? STATE_IDLE : STATE_DATA_NEEDED;
}
...
uint32_t get_stream_time(void) {
	uint32_t chunk_start_time = (str_ctx.next_chunk - 2) *
...
uint32_t get_stream_time_ms(void) {
	return (get_stream_time() * 1000) / TICKS_PER_SEC;
}
...
static const char *state_strings[] = { "IDLE", "BUFFERING", "DATA NEEDED", "READING" };
...
int main(int argc, const char* argv[]) {
	...
	int sample_rate = getSPUSampleRate(str_ctx.sample_rate);
	...
	while (1) {
		...
		FntPrint(-1, "[START]      %s\n", str_ctx.last_paused ? "RESUME" : "PAUSE");
